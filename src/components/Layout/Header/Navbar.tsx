  ...
  return (
    <nav className="fixed w-full z-50 top-0 flex justify-center transition-all duration-500">
      ...
      >
        ...
        {/* Mobile Menu Button */}
        <button
          className="md:hidden text-2xl text-white hover:text-blue-300 transition-transform active:scale-90 z-60"
          onClick={() => setOpen(!open)}
        >
          {open ? "✕" : "☰"}
        </button>

        {/* Navigation Menu - FIXED FOR MOBILE */}
        <div
          className={`md:flex md:items-center md:gap-2
            ${
              open
                ? "fixed inset-0 top-0 left-0 w-full h-screen bg-black/95 backdrop-blur-lg pt-20 px-4 overflow-y-auto"
                : "hidden"
            }
            md:static md:bg-transparent md:backdrop-blur-none md:overflow-visible
        ...
        >
          <ul className="flex flex-col md:flex-row md:items-center md:gap-2 font-semibold text-white py-4 md:py-0">
            ...
            {/* SERVICES DROPDOWN - FIXED FOR MOBILE */}
            <li
              className="relative"
              onMouseEnter={() => window.innerWidth >= 768 && setServicesOpen(true)}
              onMouseLeave={() => window.innerWidth >= 768 && setServicesOpen(false)}
            >
              <button
                className="flex items-center gap-1 w-full px-6 py-3 text-lg hover:text-blue-300 transition-all text-left"
                onClick={handleServicesClick}
              >
                Services
                <ChevronDown
              ...
              {/* DROPDOWN CONTENT - FIXED FOR MOBILE */}
              <div
                className={`
                  ${
                    servicesOpen
                      ? "block max-h-screen opacity-100 overflow-y-auto"
                      : "hidden max-h-0 opacity-0"
                  }
                  md:absolute md:top-full md:left-1/2 md:-translate-x-1/2 md:mt-2
                  md:w-[95vw] md:max-w-6xl
                  transition-all duration-300 ease-in-out
                  bg-white text-black rounded-2xl shadow-2xl z-50
                `}
              >
                <div className="p-6 md:p-8 bg-gray-50">
                  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-6">
                      ...
                      <div key={idx} className="">
                        ...
                        <ul className="list-disc list-inside text-gray-700 text-xs md:text-sm leading-snug space-y-1">
                          {category.items.map((item, i) => (
                            <li key={i}>
                              <button
                                className="cursor-pointer hover:text-blue-600 text-left w-full"
                                onClick={() => handleItemClick(category.title, item)}
                              >
                                {item}
                              </button>
